...
export default class IncomeType extends React.Component{
   constructor(props) {
    super(props);
 ...
 getData = async () => {
  ...
  validation= ()=>{
    let status = true
    console.log(status)
    if(this.state.incometype == null || this.state.incometype == ""){
      this.setState({isincometype:true})
      status = false
    }
  ...
  BtnSave = () =>{
    console.log("Saving income type")
    ...
    fetch('http://192.168.43.179:8000/incometype', {
      method: 'POST',
      headers: {
    ...
    return(
      <>
        <Appbar.Header style={{backgroundColor:"#ff4800", borderColor:"#ff4800",elevation:0}}>
          <Appbar.BackAction onPress={()=>{this.props.navigation.navigate("Dpick")}} />
          <Appbar.Content title="Income Type"  />
        </Appbar.Header>
        ...
        showsHorizontalScrollIndicator={false}>
          ...
          <View style={{width:"100%"}}>
            ...
            />
            {this.state.isincometype == true ? <Text style={{color:'red', marginLeft:17}}>Income Type should not be empty</Text> : null}
