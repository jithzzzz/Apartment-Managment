...
export default function ExpAdd(props){
  ...
  const savedata = () =>{
    setIsLoading(true);
    fetch('http://192.168.43.179:8000/expensesave', {
      method: 'POST',
      headers: {
      ...
      body:JSON.stringify({
        ...
        .then((responseJson) => {
          ...
          if(responseJson.Status == "OK"){
            setIsLoading(false);
            setVisible(true)
            setMessage("New Expense Record Added !!!")
          }else{
  ...
  return (
    <>
      ...
      {isdate == true ? <Text style={{color:'red', marginLeft:17}}>Invalid date</Text> : null}
            ...
            />
            {isinctype == true ? <Text style={{color:'red', marginLeft:17}}>Expense Type should not be empty</Text> : null}
